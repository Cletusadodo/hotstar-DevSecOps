pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {
            steps {
                git branch: 'main', credentialsId: 'git-token', url: 'https://github.com/Cletusadodo/hotstar-DevSecOps.git'
            }
        }
        ...
        stage("TRIVY") {
            steps {
                sh "trivy image cletusadodo/hotstar:latest > trivyimage.txt"
            }
        }

        stage('Deploy to container') {
            steps {
                sh 'docker run -d --name hotstar -p 3000:3000 cletusadodo/hotstar:latest'
            }
        }
    }
        ...
        always {
            script {
                ...
                emailext(
                    ...
                    """,
                    to: 'adodocletus9@gmail.com.com',
                    from: 'adodocletus9@gmail.com.com',
                    replyTo: 'adodocletus9@gmail.com.com',
                    mimeType: 'text/html',
                    attachmentsPattern: 'trivyfs.txt,trivyimage.txt,gitleaks-report.json'
                )
            }
        }
    }
